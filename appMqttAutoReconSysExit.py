...
class MqttHandler:
    def __init__(self, broker_host, broker_port, client_id, topic_base, username=None, password=None):
        ...
        # Enable Paho's automatic reconnect logic with delays
    ...
    def _on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            mqtt_connected = False
            # If authentication/authorization fails (rc 4 or 5), consider logging a critical error.
            # You might want to trigger a sys.exit(1) here too if these are unrecoverable.

    def _on_disconnect(self, client, userdata, rc):
        """Callback for when the client disconnects from the server."""
        global mqtt_connected
        print "[MQTT Sender] MQTT Disconnected with code {0}. Paho will attempt to reconnect.".format(rc)
        mqtt_connected = False
        # Paho's reconnect_delay_set handles reconnection attempts automatically here.

    def connect(self):
        """
        Attempts to initiate an MQTT connection using connect_async.
        This is primarily for the *initial* connection attempt.
        Paho's internal loop (started in __init__) and reconnect_delay_set()
        will manage subsequent reconnections automatically.
        """
...
def main():
    ...
    # for all subsequent reconnections.
            ...
            try: # Inner try block for exceptions within each loop iteration
                ...
                if network_ok:
                    if not mqtt_connected:
                        # If not connected, Paho's internal loop is already trying to reconnect.
                        # We just increment a counter to detect if this state persists for too long.
                        ...
                        if connection_wait_cycles >= MAX_CONSECUTIVE_WAIT_CYCLES:
                            ...
                            print "[MQTT Sender] CRITICAL: MQTT connection persistently failing after {0} cycles. Exiting to trigger system restart.".format(MAX_CONSECUTIVE_WAIT_CYCLES)
